...
class ReviewerErrorHandler:
    ...
    def __call__(self, exception: Exception):
        try:
            ...
            try:
                ...
                self._error_prompt_factory.create(
                    e,
                    _("Encountered an error while trying to connect to AMBOSS"),
                    _("AMBOSS - Connection Error"),
                )
        ...
        # Most SSL errors seems to be related to timeout, treat them as such.
            ...
            self._error_prompt_factory.create(e)
...
class ReviewerCardPhraseUpdater:
    ...
    def _on_card_updated(self, *args):  # new-style hooks pass card object, old don't
        ...
        for card_id in self._schedule_service.get_upcoming_card_ids():
            self._dispatch_worker(
                self._reviewer.mw.col.getCard(card_id).note(), self._cache_phrase_pairs
            )
    ...
    def _toggle_highlights(self, state: bool):
        ...
        if not state:
            self._reviewer.web.eval(
                "ambossAddon.tooltip.phraseMarker.hideAll(); ambossAddon.tooltip.tooltips.hideAll();"
            )
        else:
    ...
    def _register_changes(self):
        self._activity_service.register_activity(
            "config.change", dict(self._addon_config)
        )
