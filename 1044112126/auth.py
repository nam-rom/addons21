...
from urllib3 import HTTPResponse  # type: ignore [import]
...
class GraphQLLoginErrorParser:
    ...
    def _parse_from_list(self, error: List) -> str:
        ...
        if error_code == GraphQLErrorCode.not_authorized.value:
            return _(
                "Looks like you entered the wrong email or password. (Error code: {})".format(
                    error_code
                )
            )
        else:
...
class FormRegisterClient:
    ...
    def register(self, username: str, password: str) -> FormRegisterResponse:
        try:
            ...
            return FormRegisterResponse(
                ...
                error=_(
                    ...
                    "Please check if you received a confirmation email or try again later."
                ),
            )
        except Exception as e:
            self._error_prompt_factory.create(
                e,
                _("Encountered an error during registration"),
            )
            return FormRegisterResponse(
                success=False,
                error=_(
        ...
        return (
            ...
            if self._url_fragment_register_success in response.geturl()
    ...
    def login(self, username: str, password: str) -> dict:
        try:
            graphql_response = self._graphql_query_resolver.resolve(
                f"""\
mutation ankiLogin {{
  login(login: "{username}", password: "{password}", deviceId: "anki") {{
    token
    user {{
      eid
      firstName
