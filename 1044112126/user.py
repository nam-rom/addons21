...
class LoginStateResolver:
    ...
    def is_logged_in(self) -> bool:
        ...
        try:
            self._query_resolver.resolve(check_token_query)
...
class ArticleAccessResolver:
    ...
    def article_access(
        ...
        """Optimistic, i.e., if article access can't be determined, assumes the user has platform access."""
        default = (
            ArticleAccessTarget.public_article
            if is_logged_in is False
            else ArticleAccessTarget.platform_article
        )
        try:
            data = self._article_access_client.access_target()
            target = ArticleAccessTarget[data.get("target", default.value)]  # type: ignore[union-attr]
        except:
            return default
        return target
