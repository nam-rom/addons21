...
class UpdateNotificationDialog(QDialog):
    def __init__(self, update_data: UpdateData, parent: QWidget = None):
    ...
    def _setup_ui(self):
        ...
        body_label = QLabel(
            _(
                "A new update of the AMBOSS add-on is available. You can see what’s new listed below. Would you like to install it now?"
            )
        )
        ...
        self._button_box.setStandardButtons(
            QDialogButtonBox.Cancel | QDialogButtonBox.Ok  # type: ignore
        )
...
class UpdateUIHandler(QObject):
    ...
    def start_download(self, update_data: UpdateData):
        ...
        self._update_progress_dialog = UpdateProgressDialog(
            maximum=update_data.bytes, parent=self._main_window
        )
