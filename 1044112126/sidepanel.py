...
from aqt.utils import isMac, openLink
...
from .gui.forms.anki21.navbar import Ui_NavBar  # type: ignore[import]
from .gui.forms.anki21.sidepanel import Ui_SidePanel  # type: ignore[import]
...
class _SplitterHandleEventFilter(QObject):
    ...
    def eventFilter(self, object: QObject, event: QEvent) -> bool:
        if event.type() == QEvent.MouseButtonDblClick:
            ...
            and event.button() == Qt.LeftButton  # type: ignore[attr-defined]
        ):
...
"""
    ...
    def _translate_ui(self):
        ...
        self._form.label_title.setText(_("AMBOSS viewer (beta)"))
...
class SidePanel(QWidget):
    ...
    def event(self, event: QEvent) -> bool:
        ...
        if event.type() == QEvent.KeyPress and (
            event.key() == Qt.Key_AltGr or event.key() == 16777251  # type: ignore[attr-defined]
        ):
...
class SidePanelController(QObject):
    ...
    def _setup_style(self):
        self._splitter.main_handle.setAttribute(Qt.WA_Hover)
    ...
    def on_login_error(self, exception: Exception):
        self._error_prompt_factory.create(
            exception=exception,
            message=_("Encountered an error while trying to log into AMBOSS sidebar."),
        )
