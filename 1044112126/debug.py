...
from typing import TYPE_CHECKING, Deque, Optional, Sequence, Tuple, Union
...
class ErrorSubmitter:
    def __init__(
        self, profile: ProfileAdapter, sentry_dsn: str, version: str, stage: str
    ):
...
class DebugInfo:
    ...
    def anki(self) -> dict:
        ...
        from anki.utils import isMac, isWin, versionWithBuild
...
class ErrorPromptFactory:
    _default_message = _("Encountered an unexpected error in the AMBOSS add-on")
    _default_title = _("AMBOSS - Unexpected error")
...
class ErrorPrompt(QDialog):
    ...
    def __init__(
        ...
        self._main_window = main_window
        self._debug_data = debug_service.get_for_machine()
        self._error_submitter = error_submitter
        self._exception = exception
        self._message = message
        self._title = title
        self._max_message_chars = max_message_chars
        self._message_label_string = (
        ...
        self.text_edit = ErrorTextEdit(self._max_message_chars, self)
    ...
    def _setup_ui(self):
        ...
        self._message_label = QLabel(
            self._message_label_string.format(chars=self._max_message_chars)
        )
    ...
    def _on_message_char_count_updated(self, char_count: int):
        self._message_label.setText(
            self._message_label_string.format(
                chars=self._max_message_chars - char_count
            )
        )
