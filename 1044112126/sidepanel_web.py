...
# (the latest git master of PyQt5-stubs comes with stubs for QtWebEngine,
# but the type definitions are for Qt 5.15.x)
from PyQt5.QtWebEngineWidgets import (  # type: ignore[import]
    QWebEnginePage,
    QWebEngineProfile,
    QWebEngineSettings,
)
...
class ExternallyOpeningWebPage(QWebEnginePage):
    ...
    def __init__(
        self, url_redirector: SidePanelURLRedirector, parent: "SidePanelWebPage"
    ) -> None:
...
class SidePanelWebPage(WebPage):
    ...
    def createWindow(self, type: QWebEnginePage.WebWindowType) -> QWebEnginePage:
        ...
        return ExternallyOpeningWebPage(
            url_redirector=self._url_redirector, parent=self
        )
